...
def get_sae_activations(
    ...
    return activations.sum(dim=0).sum(dim=0) #[1,dSAE]
...
def compute_all_class_activations(
    ...
    for class_idx, classname in enumerate(tqdm(classnames)):
        ...
        class_activation_counts[class_idx] = class_counts/len(class_counts)
...
def main(
    ...
    if dataset_name in ['custom_data']:
        classnames, data_by_class = load_and_organize_dataset_csv(dataset_name)
    else:
        classnames, data_by_class = load_and_organize_dataset(dataset_name)
    
    #classnames, data_by_class = load_and_organize_dataset(dataset_name)
    print(f"Data by class loaded. Number of classes: {len(classnames)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_name", type=str, default="hehr")
    parser.add_argument(
