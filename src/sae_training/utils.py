...
def process_model_inputs(
    batch: Dict, vit: HookedVisionTransformer, device: str,
    ...
    pixel_values = torch.stack(processed_images, dim=0).to(device)

    # Return it in dict form, so your code can remain consistent:
    return pixel_values


def get_model_activations(
    model: HookedVisionTransformer, inputs: torch.Tensor, block_layer, module_name, class_token
) -> torch.Tensor:
    ...
    #print(f'This is input shape: {inputs.shape}')
    #print(f'This is activations shape: {activations.shape}')
